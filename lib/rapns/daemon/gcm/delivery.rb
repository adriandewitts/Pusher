      ...
      class Delivery < Rapns::Daemon::Delivery
        ...
        def ok(response)
          ...
          if body['failure'].to_i == 0            
            Rapns::Daemon.logger.info("[#{@app.name}] #{@notification.id} sent to #{@notification.devices.join(', ')}")
          else
        ...
        def handle_errors(response, body)
          ...
          body['results'].each_with_index do |result, i|
            if result['error']
              ...
              error_devices << @notification.devices[i]
            end 
          end
          
          @notification.app.devices.in(device: error_devices).update_all(enabled: false) unless error_devices.blank?
        ...
        def build_new_notification(response, idxs)
          ...
          notification.attributes = @notification.attributes.slice('app_id', 'collapse_key', 'delay_while_idle')
          notification.payload = @notification.payload
          notification.devices = idxs.map { |i| @notification.devices[i] }
        ...
        def describe_errors(errors)
          description = if errors.size == @notification.devices.size
