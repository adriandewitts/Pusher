require 'spec_helper'
require 'models/notification_shared.rb'

describe Apns::Notification do
  it_should_behave_like 'an Notification subclass'
  let(:notification_class) { Apns::Notification }
  let(:notification) { notification_class.new }
  let(:data_setter) { 'attributes_for_device=' }
  let(:data_getter) { 'attributes_for_device' }

  it "should validate the format of the device_token" do
    notification = Apns::Notification.new(:devices => ["{$%^&*()}"])
    notification.valid?.should be_false
    puts notification.errors[:devices] 
    notification.errors[:devices].include?("is invalid").should be_true
  end 

  it "should validate the length of the binary conversion of the notification" do
    notification.devices = ["a" * 64] 
...
describe Apns::Notification, 'MDM' do
  let(:magic) { 'abc123' }
  let(:notification) { Apns::Notification.new }

  xit 'includes the mdm magic in the payload' do
    notification.mdm = magic
    notification.as_json.should == {'mdm' => magic}
  end

  xit 'does not include aps attribute' do
...
describe Apns::Notification, 'content-available' do
  let(:notification) { Apns::Notification.new }
...
describe Apns::Notification, "to_binary" do
  it "should correctly convert the notification to binary" do
    ...
    notification.app = Apns::App.create!(:name => 'my_app', :environment => 'development', :certificate => TEST_CERT)
    notification.save!
...
describe Apns::Notification, "bug #31" do
...
describe Apns::Notification, "bug #35" do
  it "should limit payload size to 256 bytes but not the entire packet" do
    notification = Apns::Notification.new do |n|
      n.device_token = "a" * 64
      n.alert = "a" * 210
      n.app = Apns::App.create!(:name => 'my_app', :environment => 'development', :certificate => TEST_CERT)
    end
...
describe Apns::Notification, "multi_json usage" do
  describe Apns::Notification, "alert" do
    it "should call MultiJson.load when multi_json version is 1.3.0" do
      notification = Apns::Notification.new(:alert => { :a => 1 }, :alert_is_json => true)
